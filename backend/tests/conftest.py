...
def _ensure_builtin_form_templates():
    """Phase 3.4b: `form_templates` now lives in PostgreSQL. This fixture
    used to seed built-in templates directly into Mongo via pymongo, which
    regenerated the retired collection. The runtime app is now responsible
    for seeding form_templates into PG (via routers.forms_protocols or the
    dedicated seed pathway); tests that need built-ins should call the
    HTTP surface instead of writing raw Mongo docs."""
    yield
